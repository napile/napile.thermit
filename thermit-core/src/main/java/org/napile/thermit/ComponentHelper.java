...
import org.napile.thermit.taskdefs.Definer;
import org.napile.thermit.taskdefs.Typedef;
 ...
 * ( for example in an app embedding thermit )
 *
 * @since Ant1.6
 */
public class ComponentHelper
{
	 ...
	 */
	public static final String COMPONENT_HELPER_REFERENCE = "thermit.ComponentHelper";
	...
	/**
	 * special name of thermit's property task -{@value}. There is some
	 ...
	 * @return the thermit definition or null if not present.
	 */
	...
	/**
	 * This method is initialization code implementing the original thermit component
	 * loading from /org/napile/tools/thermit/taskdefs/default.properties
	 * and /org/napile/tools/thermit/types/default.properties.
	 */
	...
	public Task createTask(String taskType) throws BuildException
	{
		...
		if(task == null && taskType.equals(ANT_PROPERTY_TASK))
		{
			...
			addTaskDefinition(ANT_PROPERTY_TASK, org.napile.thermit.taskdefs.Property.class);
	...
	public String getElementName(Object o, boolean brief)
	{
		...
		synchronized(antTypeTable)
		{
			...
			{
				if(elementClassname.equals(def.getClassName()) && (elementClass == def.getExposedClass(project)))
				{
	...
	private void updateDataTypeDefinition(AntTypeDefinition def)
	{
		...
		synchronized(antTypeTable)
		{
			...
			if(old != null)
			{
				...
				project.log("Trying to override old definition of " + (isTask ? "task " : "datatype ") + name, (def.similarDefinition(old, project)) ? Project.MSG_VERBOSE : Project.MSG_WARN);
			}
	...
	public String diagnoseCreationFailure(String componentName, String type)
	{
		...
		else
		{
			...
			antTask = classname.startsWith("org.napile.thermit.");
			boolean optional = classname.startsWith("org.napile.thermit.taskdefs.optional");
			optional |= classname.startsWith("org.napile.thermit.types.optional");
