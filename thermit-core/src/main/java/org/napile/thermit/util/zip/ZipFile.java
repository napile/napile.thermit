...
package org.napile.thermit.util.zip;
...
import static org.napile.thermit.util.zip.ZipConstants.DWORD;
import static org.napile.thermit.util.zip.ZipConstants.SHORT;
import static org.napile.thermit.util.zip.ZipConstants.WORD;
import static org.napile.thermit.util.zip.ZipConstants.ZIP64_MAGIC;
import static org.napile.thermit.util.zip.ZipConstants.ZIP64_MAGIC_SHORT;
 ...
 * <code>org.napile.thermit.util.zip.ZipEntry</code> instead of
 ...
 * <code>org.napile.thermit.util.zip.ZipEntry</code> instances.</li>
...
public class ZipFile
{
	 ...
	 */
	private final Map<ZipEntry, OffsetEntry> entries = new LinkedHashMap<ZipEntry, OffsetEntry>(HASH_SIZE);

	/**
	 * Maps String to ZipEntrys, name -> actual entry.
	 */
	private final Map<String, ZipEntry> nameMap = new HashMap<String, ZipEntry>(HASH_SIZE);
	...
	{
		ZipEntry[] allEntries = entries.keySet().toArray(new ZipEntry[0]);
	...
	{
		boolean found = tryToLocateSignature(MIN_EOCD_SIZE + ZIP64_EOCDL_LENGTH, MAX_EOCD_SIZE + ZIP64_EOCDL_LENGTH, ZipOutputStream.ZIP64_EOCD_LOC_SIG);
		if(!found)
		{
			// not a ZIP64 archive
	...
	private void positionAtCentralDirectory64() throws IOException
	{
		...
		if(sig[POS_0] != ZipOutputStream.ZIP64_EOCD_SIG[POS_0] || sig[POS_1] != ZipOutputStream.ZIP64_EOCD_SIG[POS_1] || sig[POS_2] != ZipOutputStream.ZIP64_EOCD_SIG[POS_2] || sig[POS_3] != ZipOutputStream.ZIP64_EOCD_SIG[POS_3])
		{
	...
	private void resolveLocalFileHeaderData(Map<ZipEntry, NameAndComment> entriesWithoutUTF8Flag) throws IOException
	{
		...
		// directory order
		Map<ZipEntry, OffsetEntry> origMap = new LinkedHashMap<ZipEntry, OffsetEntry>(entries);
		entries.clear();
