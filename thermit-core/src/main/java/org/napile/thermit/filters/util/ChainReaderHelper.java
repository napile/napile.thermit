 ...
 *
 */
package org.napile.thermit.filters.util;
...
import org.napile.thermit.filters.BaseFilterReader;
import org.napile.thermit.filters.ChainableReader;
import org.napile.thermit.types.AntFilterReader;
import org.napile.thermit.types.FilterChain;
import org.napile.thermit.types.Parameter;
import org.napile.thermit.types.Parameterizable;
...
public final class ChainReaderHelper
{
	...
	public Reader getAssembledReader() throws BuildException
	{
		...
		{
			final FilterChain filterchain = filterChains.elementAt(i);
		...
		if(filtersCount > 0)
		{
			...
			try
			{
				for(int i = 0; i < filtersCount; i++)
				{
					...
					{
						instream = expandReader((AntFilterReader) finalFilters.elementAt(i), instream, classLoadersToCleanUp);
					}
		...
		if(className != null)
		{
			try
			{
				...
				if(clazz != null)
				{
					...
					Reader instream = (Reader) constructors[j].newInstance((Object[]) rdr);
					setProjectOnObject(instream);
