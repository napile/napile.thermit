...
import org.napile.thermit.types.selectors.TokenizedPath;
import org.napile.thermit.types.selectors.TokenizedPattern;
...
public class DirectoryScanner implements FileScanner, SelectorScanner, ResourceFactory
{
	...
	private void scandir(File dir, TokenizedPath path, boolean fast, String[] newfiles, LinkedList<String> directoryNamesFollowed)
	{
	...
	private void accountForIncludedDir(TokenizedPath name, File file, boolean fast, String[] children, LinkedList<String> directoryNamesFollowed)
	{
		processIncluded(name, file, dirsIncluded, dirsExcluded, dirsDeselected);
		if(fast && couldHoldIncluded(name) && !contentsExcluded(name))
		{
	...
	private void processIncluded(TokenizedPath path, File file, Vector<String> inc, Vector<String> exc, Vector<String> des)
	{
	...
	private boolean couldHoldIncluded(TokenizedPath tokenizedName)
	{
		...
		for(Iterator<TokenizedPath> iter = includeNonPatterns.values().iterator(); iter.hasNext(); )
		{
			if(couldHoldIncluded(tokenizedName, iter.next().toPattern()))
			{
				return true;
			}
		}
	 ...
	 * @since Ant 1.8.0
	 */
	private boolean causesIllegalSymlinkLoop(String dirName, File parent, LinkedList<String> directoryNamesFollowed)
	{
		try
		{
