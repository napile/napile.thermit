...
import org.napile.thermit.util.ReflectUtil;
import org.napile.thermit.util.VectorSet;
import org.napile.thermit.launch.Locator;

/**
 * Used to load classes within thermit with a different classpath from
 * that used to start thermit. Note that it is possible to force a class
...
public class AntClassLoader extends ClassLoader implements SubBuildListener
{
	...
	public InputStream getResourceAsStream(String name)
	{
		...
		else
		{
			...
			if(resourceStream != null)
			{
				log("ResourceStream for " + name + " loaded from thermit loader", Project.MSG_DEBUG);
			}
		}
	...
	public URL getResource(String name)
	{
		...
		else
		{
			...
			while(e.hasMoreElements() && url == null)
			{
				...
				if(url != null)
				{
					log("Resource " + name + " loaded from thermit loader", Project.MSG_DEBUG);
				}
			}
		}
	...
	protected synchronized Class<?> loadClass(String classname, boolean resolve) throws ClassNotFoundException
	{
		...
		if(isParentFirst(classname))
		{
			...
			{
				theClass = findClass(classname);
				log("Class " + classname + " loaded from thermit loader " + "(parentFirst)", Project.MSG_DEBUG);
			}
		}
		else
		{
			...
			{
				theClass = findClass(classname);
				log("Class " + classname + " loaded from thermit loader", Project.MSG_DEBUG);
			}
			catch(ClassNotFoundException cnfe)
			{
				if(ignoreBase)
	...
	static
	{
		if(JavaEnvUtils.isAtLeastJavaVersion(JavaEnvUtils.JAVA_1_5))
		{
			try
			{
				subClassToLoad = Class.forName("org.napile.thermit.loader.AntClassLoader5");
			}
	...
	public static AntClassLoader newAntClassLoader(ClassLoader parent, Project project, Path path, boolean parentFirst)
	{
		...
		{
			return (AntClassLoader) ReflectUtil.newInstance(subClassToLoad, CONSTRUCTOR_ARGS, new Object[]{
					parent,
					project,
					path,
