...
public class Path extends DataType implements Cloneable, ResourceCollection
{
	 ...
	 */
	public static final Path systemClasspath = new Path(null, System.getProperty("java.class.path"));
	...
	public void addExisting(Path source, boolean tryUserDir)
	{
		...
		for(int i = 0; i < list.length; i++)
		{
			...
			{
				setLocation(f);
				log("adding " + f + " which contains wildcards and may not" + " do what you intend it to do depending on your OS or" + " version of Java", Project.MSG_VERBOSE);
			}
			else
			{
	...
	public String[] list()
	{
		...
		return assertFilesystemOnly(union) == null ? new String[0] : union.list();
	}
	...
	{
		return isReference() ? getCheckedRef().toString() : union == null ? "" : union.toString();
	}
	...
	private Path concatSpecialPath(String defValue, Path p)
	{
		...
		String o = getProject() != null ? getProject().getProperty(MagicNames.BUILD_SYSCLASSPATH) : System.getProperty(MagicNames.BUILD_SYSCLASSPATH);
		if(o != null)
		{
			order = o;
		}
		...
		else
		{
			...
			{
				log("invalid value for " + MagicNames.BUILD_SYSCLASSPATH + ": " + order, Project.MSG_WARN);
			}
			result.addExisting(this);
			result.addExisting(p, true);
		}
