 ...
 * <p/>
 * <p>See {@link
 * org.napile.thermit.taskdefs.optional.XMLValidateTask
...
public class XMLCatalog extends DataType implements Cloneable, EntityResolver, URIResolver
{
	 ...
	 * classpath.
	 */
	public static final String APACHE_RESOLVER = "org.napile.thermit.types.resolver.ApacheCatalogResolver";

	/**
	 * Resolver base class
	 */
	public static final String CATALOG_RESOLVER = "org.napile.xml.resolver.tools.CatalogResolver";
	 ...
	 * @throws BuildException if this is a reference and no nested
	...
	public InputSource resolveEntity(String publicId, String systemId) throws SAXException, IOException
	{
		...
		if(inputSource == null)
		{
			log("No matching catalog entry found, parser will use: '" + systemId + "'", Project.MSG_DEBUG);
		}
	...
	private CatalogResolver getCatalogResolver()
	{
		...
		if(catalogResolver == null)
		{
			...
			catch(Throwable ex)
			{
				...
				{
					log("Warning: XML resolver not found; external catalogs" + " will be ignored", Project.MSG_WARN);
				}
	...
	private InputSource filesystemLookup(ResourceLocation matchingEntry)
	{
		...
		catch(MalformedURLException ex)
		{
			...
			{
				log("uri : '" + uri + "' matches a readable file", Project.MSG_DEBUG);
				try
				{
			...
			{
				log("uri : '" + uri + "' does not match a readable file", Project.MSG_DEBUG);
			...
			if(fileName != null)
			{
				...
				if(resFile.exists() && resFile.canRead())
				{
					try
					{
						...
						log("catalog entry matched a readable file: '" + sysid + "'", Project.MSG_DEBUG);
					}
	...
	private InputSource classpathLookup(ResourceLocation matchingEntry)
	{
		...
		if(is != null)
		{
			...
			source.setSystemId(sysid);
			log("catalog entry matched a resource in the classpath: '" + sysid + "'", Project.MSG_DEBUG);
		}
	...
	private class InternalResolver implements CatalogResolver
	{
		...
		{
			log("Apache resolver library not found, internal resolver will be used", Project.MSG_VERBOSE);
		}
	...
	private class ExternalResolver implements CatalogResolver
	{
		...
		public ExternalResolver(Class<?> resolverImplClass, Object resolverImpl)
		{
			...
			try
			{
				...
				resolveEntity = resolverImplClass.getMethod("resolveEntity", new Class[]{
						String.class,
						String.class
				});

				resolve = resolverImplClass.getMethod("resolve", new Class[]{
						String.class,
						String.class
				});
			}
			...
			log("Apache resolver library found, xml-commons resolver will be used", Project.MSG_VERBOSE);
		}
		...
		public InputSource resolveEntity(String publicId, String systemId)
		{
			...
			if(matchingEntry != null)
			{
				...
				if(result == null)
				{
					try
					{
						result = (InputSource) resolveEntity.invoke(resolverImpl, new Object[]{
								publicId,
								systemId
						});
					}
		...
		private void processExternalCatalogs()
		{
			...
			if(!externalCatalogsProcessed)
			{
				...
				try
				{
					setXMLCatalog.invoke(resolverImpl, new Object[]{XMLCatalog.this});
				}
				catch(Exception ex)
				{
					throw new BuildException(ex);
