...
import org.napile.thermit.types.TarFileSet;
...
public class Archives extends DataType implements ResourceCollection, Cloneable
{
	...
	public Iterator<Resource> iterator()
	{
		...
		for(Iterator<ArchiveFileSet> i = grabArchives(); i.hasNext(); )
		{
			l.addAll(CollectionUtils.asCollection(i.next().iterator()));
		}
		return l.iterator();
	}
	...
	{
		if(zips.getResourceCollections().size() > 0 || tars.getResourceCollections().size() > 0)
		{
