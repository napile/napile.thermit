...
import org.napile.thermit.util.LoaderUtils;
...
public class ProjectHelperRepository
{
	...
	private static final String DEBUG_PROJECT_HELPER_REPOSITORY = "thermit.project-helper-repo.debug";
	...
	private void collectProjectHelpers()
	{
		...
		try
		{
			...
			if(classLoader != null)
			{
				Enumeration<URL> resources = classLoader.getResources(ProjectHelper.SERVICE_ID);
	 ...
	 *
	 * @param helperClassName the fully qualified name of the helper
	 * @throws BuildException if the class cannot be loaded or if there is no constructor
	 *                        with no argument
	 * @since Ant 1.8.2
	 */
	 ...
	 * before the default one (ProjectHelper2)
	 *
	 * @param helperClass the class of the helper
	...
	private Constructor<? extends ProjectHelper> getProjectHelperBySystemProperty()
	{
		...
		{
			System.err.println("Unable to load ProjectHelper class \"" + helperClass + " specified in system property " + ProjectHelper.HELPER_PROPERTY + " (" + e.getMessage() + ")");
			if(DEBUG)
			{
				e.printStackTrace(System.err);
			}
	...
	private Constructor<? extends ProjectHelper> getProjectHelperByService(InputStream is)
	{
		...
		{
			System.out.println("Unable to load ProjectHelper from service " + ProjectHelper.SERVICE_ID + " (" + e.getMessage() + ")");
			if(DEBUG)
			{
				e.printStackTrace(System.err);
			}
	 ...
	 *
	 * @param helperClass The name of the class to create an instance of. Must not be
	 *                    <code>null</code>.
	 * @return the constructor of the specified class.
	 * @throws BuildException if the class cannot be found or if a constructor with no
	...
	private static class ConstructingIterator implements Iterator<ProjectHelper>
	{
		...
		public ProjectHelper next()
		{
			...
			{
				// last but not least, thermit default project helper
				empty = true;
