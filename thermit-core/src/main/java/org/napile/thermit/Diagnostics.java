 ...
 * jar existing in thermit.home/lib and the jar versions...
 *
 * @since Ant 1.5
 */
public final class Diagnostics
{
	 ...
	 *
	 * @return the list of jar files existing in thermit.home/lib or
	...
	public static void doReport(PrintStream out, int logLevel)
	{
		...
		header(out, "USER_HOME/.thermit/lib jar listing");
		doReportUserHomeLibraries(out);

		header(out, "Tasks availability");
		doReportTasksAvailability(out);

		header(out, "org.napile.env.Which diagnostics");
		doReportWhich(out);
	...
	/**
	 * Report the content of ~/.thermit/lib directory
	 *
	 * @param out the stream to print the content to
	 */
	...
	/**
	 * Call org.napile.env.Which if available
	 *
	 * @param out the stream to print the content to.
	 */
	private static void doReportWhich(PrintStream out)
	{
		...
		try
		{
			Class<?> which = Class.forName("org.napile.env.Which");
		...
		catch(ClassNotFoundException e)
		{
			...
			out.println("Download it at http://xml.napile.org/commons/");
		}
		...
		if(error != null)
		{
			out.println("Error while running org.napile.env.Which");
	...
	private static void doReportTempDir(PrintStream out)
	{
		...
		try
		{
			...
			{
				out.println(", but seems to be full.  Wrote " + (TEST_FILE_SIZE * KILOBYTE) + "but could only read " + total + " bytes.");
			}
			else
			{
				out.println();
	...
	private static void doReportProxy(PrintStream out)
	{
		...
		final String proxyDiagClassname = "org.napile.thermit.util.java15.ProxyDiagnostics";
		try
		{
