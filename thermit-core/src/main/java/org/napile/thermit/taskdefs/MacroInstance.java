...
import org.napile.thermit.DynamicAttribute;
...
/**
 * The class to be placed in the thermit type definition.
	...
	private UnknownElement copy(UnknownElement ue, boolean nested)
	{
		...
		while(e.hasMoreElements())
		{
			...
			else
			{
				...
				if(list != null)
				{
					for(Iterator<UnknownElement> i = list.iterator(); i.hasNext(); )
					{
						UnknownElement child = copy(i.next(), true);
						rc.addChild(child.getWrapper());
						ret.addChild(child);
					}
				}
