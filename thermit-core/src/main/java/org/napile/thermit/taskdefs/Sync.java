...
import org.napile.thermit.types.AbstractFileSet;
...
public class Sync extends Task
{
	...
	public void execute() throws BuildException
	{
		...
		if(!myCopy.getIncludeEmptyDirs() || getExplicitPreserveEmptyDirs() == Boolean.FALSE)
		{
			...
			{ // must be syncTarget.preserveEmptydirs == FALSE
	 ...
	 */
	private int[] removeOrphanFiles(Set nonOrphans, File toDir, Set preservedDirectories)
	{
		int[] removedCount = new int[]{
				0,
				0
		};
	...
	{
		int removedCount = 0;
		for(Iterator iter = preservedEmptyDirectories.iterator(); iter.hasNext(); )
		{
