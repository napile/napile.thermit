 ...
 * "thermit.coreLoader". (the core loader is used to load system thermit
 ...
 * the "thermit.loader.NAME" pattern. NAME will be used as a pathref when
 * calling taskdef.
 * <p/>
 ...
 * The typical use is:
 * <pre>
 *  &lt;path id="thermit.deps" &gt;
 *     &lt;fileset dir="myDir" &gt;
 *        &lt;include name="junit.jar, bsf.jar, js.jar, etc"/&gt;
 *     &lt;/fileset&gt;
 *  &lt;/path&gt;
 *
 *  &lt;classloader pathRef="thermit.deps" /&gt;
 *
 * </pre>
 */
public class Classloader extends Task
{
	...
	public void execute()
	{
		try
		{
			...
			{
				log("Changing the system loader is disabled " + "by build.sysclasspath=only", Project.MSG_WARN);
				return;
			}
			...
			{
				log("Referenced object is not an AntClassLoader", Project.MSG_ERR);
				return;
			}
			...
			if(acl == null)
			{
				...
				acl = AntClassLoader.newAntClassLoader((ClassLoader) parent, getProject(), classpath, parentFirst);
				...
				if(name == null)
				{
					...
					// without delegating
					acl.addLoaderPackageRoot("org.napile.thermit.taskdefs.optional");
			...
			if(existingLoader && classpath != null)
			{
				...
				for(int i = 0; i < list.length; i++)
				{
					...
					if(f.exists())
					{
						log("Adding to class loader " + acl + " " + f.getAbsolutePath(), Project.MSG_DEBUG);
