...
import org.napile.thermit.types.DirSet;
...
public class Javadoc extends Task
{
	...
	public class TagArgument extends FileSet
	{
		 ...
		 *                     should not be empty.
		 * @throws BuildException if all is specified along with
	...
	public void execute() throws BuildException
	{
		...
		@SuppressWarnings("unchecked") Vector<SourceFile> sourceFilesToDoc = (Vector<SourceFile>) sourceFiles.clone();
	...
	private void checkPackagesToDoc(Vector<String> packagesToDoc, Vector<SourceFile> sourceFilesToDoc)
	{
	...
	private void doDoclet(Commandline toExecute)
	{
		if(doclet != null)
		{
			...
			else
			{
				...
				for(Enumeration<DocletParam> e = doclet.getParams(); e.hasMoreElements(); )
				{
	...
	private void doGroup(Commandline toExecute)
	{
		...
		//   E.g., group="XSLT_Packages org.napile.xalan.xslt*,
		//                XPath_Packages org.napile.xalan.xpath*"
		if(group != null)
		{
	...
	private void doSourceAndPackageNames(Commandline toExecute, Vector<String> packagesToDoc, Vector<SourceFile> sourceFilesToDoc, boolean useExternalFile, File tmpList, BufferedWriter srcListWriter) throws IOException
	{
	...
	private String quoteString(final String str, final char delim)
	{
		...
		for(int i = 0; i < len; i++)
		{
			char c = str.charAt(i);
			if(c == delim)
			{ // can't put the non-constant delim into a case
				buf.append('\\').append(c);
				lastCharWasCR = false;
			}
			else
	...
	private void parsePackages(Vector<String> pn, Path sp)
	{
		...
		@SuppressWarnings("unchecked") Vector<DirSet> dirSets = (Vector<DirSet>) packageSets.clone();
