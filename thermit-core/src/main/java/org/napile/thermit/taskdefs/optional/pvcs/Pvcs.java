 ...
 *
 */
package org.napile.thermit.taskdefs.optional.pvcs;
...
import org.napile.thermit.taskdefs.ExecuteStreamHandler;
import org.napile.thermit.taskdefs.LogOutputStream;
import org.napile.thermit.taskdefs.LogStreamHandler;
import org.napile.thermit.taskdefs.PumpStreamHandler;
 ...
 * <br>
 * Before using this tag, the user running thermit must have access to the commands
 ...
 * discussion.
 */
public class Pvcs extends org.napile.thermit.Task
{
	 ...
	 */
	public void execute() throws org.napile.thermit.BuildException
	{
		int result = 0;
		...
		try
		{
			...
			if(!tmp.exists())
			{
				throw new BuildException("Communication between thermit and pvcs " + "failed. No output generated from executing PVCS " + "commandline interface \"pcli\" and \"get\"");
			}
		...
		try
		{
			...
			while(line != null)
			{
				...
				if(line.startsWith("\"\\")    // Checking for "\
						|| line.startsWith("\"/")  // or           "/
						// or           "X:\...
						|| (line.length() > POS_3 && line.startsWith("\"") && Character.isLetter(line.charAt(POS_1)) && String.valueOf(line.charAt(POS_2)).equals(":") && String.valueOf(line.charAt(POS_3)).equals("\\")))
				{
	 ...
	 *
	 * @param f config file - can be given absolute or relative to thermit basedir
	 */
