...
import org.napile.thermit.ExitException;
import org.napile.thermit.Project;
import org.napile.thermit.Task;
import org.napile.thermit.ExitStatusException;
...
import org.napile.thermit.types.Assertions;
import org.napile.thermit.types.Permissions;
import org.napile.thermit.types.RedirectorElement;
import org.napile.thermit.taskdefs.condition.Os;
import org.napile.thermit.util.KeepAliveInputStream;
...
public class Java extends Task
{
	...
	protected void checkConfiguration() throws BuildException
	{
		...
		{
			log("When using 'jar' attribute classpath-settings are ignored. " + "See the manual for more information.", Project.MSG_VERBOSE);
		}
		...
		else
		{
			...
			if(perm == null)
			{
				...
				log("running " + this.getCommandLine().getClassname() + " with default permissions (exit forbidden)", Project.MSG_VERBOSE);
			}
			log("Running in same VM " + getCommandLine().describeJavaCommand(), Project.MSG_VERBOSE);
		}
		setupRedirector();
	}
	 ...
	 * Ant log.
	 *
	 * @param logError get in the thermit log the messages coming from stderr
