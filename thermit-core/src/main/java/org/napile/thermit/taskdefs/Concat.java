...
import org.napile.thermit.filters.util.ChainReaderHelper;
import org.napile.thermit.types.Path;
import org.napile.thermit.types.FileSet;
import org.napile.thermit.types.FileList;
import org.napile.thermit.types.FilterChain;
...
import org.napile.thermit.types.resources.Intersect;
import org.napile.thermit.types.resources.LogOutputResource;
import org.napile.thermit.types.resources.Restrict;
import org.napile.thermit.types.resources.Resources;
import org.napile.thermit.types.resources.StringResource;
import org.napile.thermit.types.resources.selectors.Not;
...
import org.napile.thermit.types.selectors.SelectorUtils;
import org.napile.thermit.util.ConcatResourceInputStream;
import org.napile.thermit.util.FileUtils;
import org.napile.thermit.util.ReaderInputStream;
...
public class Concat extends Task implements ResourceCollection
{
	...
	private final class MultiReader<S> extends Reader
	{
		 ...
		 * @throws IOException - possibly thrown by the read for a reader
		 *                     object.
		 */
		 ...
		 * @throws IOException - possibly thrown by the reads to the
		 *                     reader objects.
		 */
	...
	private final class ConcatResource extends Resource
	{
		...
		public InputStream getInputStream() throws IOException
		{
			...
			Reader resourceReader = getFilteredReader(new MultiReader<Resource>(c.iterator(), resourceReaderFactory));
			Reader rdr;
			...
			else
			{
				...
				rdr = new MultiReader<Reader>(Arrays.asList(readers).iterator(), identityReaderFactory);
			}
			return outputEncoding == null ? new ReaderInputStream(rdr) : new ReaderInputStream(rdr, outputEncoding);
		}

		public String getName()
		{
			return resourceName == null ? "concat (" + String.valueOf(c) + ")" : resourceName;
		}
	}
	...
	private ReaderFactory<Resource> resourceReaderFactory = new ReaderFactory<Resource>()
	{
		public Reader getReader(Resource o) throws IOException
		{
			...
			return new BufferedReader(encoding == null ? new InputStreamReader(is) : new InputStreamReader(is, encoding));
		}
	};
	...
	private ResourceCollection getResources()
	{
		...
		if(dest != null)
		{
			...
			{
				throw new BuildException("Destination resource " + dest + " was specified as an input resource.");
			}
		}
