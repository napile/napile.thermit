...
public class AntStructure extends Task
{
	...
	public void execute() throws BuildException
	{
		...
		try
		{
			...
			printer.printHead(out, getProject(), new Hashtable<String, Class<?>>(getProject().getTaskDefinitions()), new Hashtable<String, Class<?>>(getProject().getDataTypeDefinitions()));

			printer.printTargetDecl(out);

			for(String typeName : getProject().getCopyOfDataTypeDefinitions().keySet())
			{
				printer.printElementDecl(out, getProject(), typeName, getProject().getDataTypeDefinitions().get(typeName));
			}
		...
		{
			throw new BuildException("Error writing " + output.getAbsolutePath(), ioe, getLocation());
		}
		finally
		{
			if(out != null)
	...
	private static class DTDPrinter implements StructurePrinter
	{
		 ...
		 */
		private void printHead(PrintWriter out, Enumeration<String> tasks, Enumeration<String> types)
		{
		...
		public void printElementDecl(PrintWriter out, Project p, String name, Class<?> element)
		{
			...
			if(org.napile.thermit.types.Reference.class.equals(element))
			{
			...
			while(e.hasMoreElements())
			{
				...
				else if(type.getSuperclass() != null && type.getSuperclass().getName().equals("java.lang.Enum"))
				{
					try
					{
						Object[] values = (Object[]) type.getMethod("values", (Class[]) null).invoke(null, (Object[]) null);
						if(values.length == 0)
						{
							sb.append("CDATA ");
						}
