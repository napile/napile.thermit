 ...
 * example usage :
 * &lt;cvsversion
 * cvsRoot=&quot;:pserver:anoncvs@cvs.napile.org:/home/cvspublic&quot;
...
public class CvsVersion extends AbstractCvsTask
{
	...
	public void execute()
	{
		...
		log("Received version response \"" + output + "\"", Project.MSG_DEBUG);
		...
		while(haveReadAhead || st.hasMoreTokens())
		{
			...
			else if(currentToken.equals("(client/server)") && cvs != null && cachedVersion != null && !client && !server)
			{
				client = server = true;
