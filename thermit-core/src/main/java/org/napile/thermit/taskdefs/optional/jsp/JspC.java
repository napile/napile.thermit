...
package org.napile.thermit.taskdefs.optional.jsp;
...
import org.napile.thermit.taskdefs.MatchingTask;
import org.napile.thermit.taskdefs.optional.jsp.compilers.JspCompilerAdapter;
import org.napile.thermit.taskdefs.optional.jsp.compilers.JspCompilerAdapterFactory;
 ...
 * attribute classpath, if the jsp compiler is not already in the thermit
 * classpath.
 * <p/>
 * <p><h4>Usage</h4>
 * <pre>
...
public class JspC extends MatchingTask
{
	...
	public void execute() throws BuildException
	{
		...
		{
			//bind to a compiler
			JspCompilerAdapter compiler = JspCompilerAdapterFactory.getCompiler(compilerName, this, al = getProject().createClassLoader(compilerClasspath));
			...
			if(compileList.size() > 0)
			{
				...
				log("Compiling " + compileList.size() + " source file" + (compileList.size() == 1 ? "" : "s") + " to " + dest);
				doCompilation(compiler);
	...
	private File getActualDestDir()
	{
		...
		{
			String path = destDir.getPath() + File.separatorChar + packageName.replace('.', File.separatorChar);
			dest = new File(path);
		}
		return dest;
	}
	...
	protected void scanDir(File srcDir, File dest, JspMangler mangler, String[] files)
	{
		...
		for(int i = 0; i < files.length; i++)
		{
			...
			{
				log("Warning: file modified in the future: " + filename, Project.MSG_WARN);
			}
	...
	private boolean isCompileNeeded(File srcFile, File javaFile)
	{
		...
		{
			shouldCompile = true;
			log("Compiling " + srcFile.getPath() + " because java file " + javaFile.getPath() + " does not exist", Project.MSG_VERBOSE);
		}
		else
		{
			...
			{
				shouldCompile = true;
				log("Compiling " + srcFile.getPath() + " because it is out of date with respect to " + javaFile.getPath(), Project.MSG_VERBOSE);
			}
			else
			{
				...
				{
					shouldCompile = true;
					log("Compiling " + srcFile.getPath() + " because java file " + javaFile.getPath() + " is empty", Project.MSG_VERBOSE);
				}
			}
		}
