...
public class Move extends Copy
{
	...
	protected void validateAttributes() throws BuildException
	{
		...
		{
			if((destFile != null && destDir != null) || (destFile == null && destDir == null))
			{
	...
	private void copyFile(File fromFile, File toFile, boolean filtering, boolean overwrite)
	{
		try
		{
			...
			getFileUtils().copyFile(fromFile, toFile, executionFilters, getFilterChains(), forceOverwrite, getPreserveLastModified(),									/* append: */ false, getEncoding(), getOutputEncoding(), getProject(), getForce());
		}
		catch(IOException ioe)
		{
	...
	protected void deleteDir(File d, boolean deleteFiles)
	{
		...
		for(int i = 0; i < list.length; i++)
		{
			...
			else if(deleteFiles && !getFileUtils().tryHardToDelete(f, performGc))
			{
	...
	{
		if(destFile.isDirectory() || filtering || getFilterSets().size() > 0 || getFilterChains().size() > 0)
		{
			return false;
		}
		...
		else if(destFile.isFile())
		{
			...
			if(destFile.getAbsolutePath().equals(sourceFile.getAbsolutePath()))
			{
				...
				log("Rename of " + sourceFile + " to " + destFile + " is a no-op.", Project.MSG_VERBOSE);
				return true;
			}
