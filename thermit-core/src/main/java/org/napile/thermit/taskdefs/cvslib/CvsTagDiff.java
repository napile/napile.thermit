...
import org.napile.thermit.taskdefs.AbstractCvsTask;
import org.napile.thermit.util.DOMElementWriter;
import org.napile.thermit.util.DOMUtils;
...
public class CvsTagDiff extends AbstractCvsTask
{
	...
	private CvsTagEntry[] parseRDiff(File tmpFile) throws BuildException
	{
		...
		try
		{
			...
			while(null != line)
			{
				line = removePackageName(line, packageNamePrefixes, packageNamePrefixLengths);
				if(line != null)
				{
					// use || in a perl like fashion
					boolean processed = doFileIsNew(entries, line) || doFileHasChanged(entries, line) || doFileWasRemoved(entries, line);
				}
	 ...
	 *
	 * @throws BuildException if a parameter is not correctly set
	 */
