...
import org.napile.thermit.util.LineOrientedOutputStreamRedirector;
import org.napile.thermit.util.StringUtils;
import org.napile.thermit.util.TeeOutputStream;
import org.napile.thermit.util.ReaderInputStream;
import org.napile.thermit.util.LeadPipeInputStream;
import org.napile.thermit.util.LazyFileOutputStream;
import org.napile.thermit.util.OutputStreamFunneler;
import org.napile.thermit.util.ConcatFileInputStream;
import org.napile.thermit.util.KeepAliveOutputStream;
...
public class Redirector
{
	 ...
	 * Create a redirector instance for the given task
	 *
	 * @param managingTask the project component for which the redirector is to work
	 * @since Ant 1.6.3
	 */
	 ...
	 *
	 * @param logError if true the standard error is sent to the Ant log system and
	 *                 not sent to output.
	 */
	 ...
	 *
	 * @param outputProperty the name of the property to be set with the task's output.
	 */
	 ...
	 *
	 * @param errorProperty the name of the property to be set with the error output.
	 */
	 ...
	 * @throws IOException if the value cannot be read form the stream.
	 */
	...
	private void outStreams()
	{
		...
		{
			String logHead = new StringBuffer("Output ").append(((appendOut) ? "appended" : "redirected")).append(" to ").toString();
	...
	private void errorStreams()
	{
		...
		{
			String logHead = new StringBuffer("Error ").append(((appendErr) ? "appended" : "redirected")).append(" to ").toString();
		...
		else if(!(logError || outputStream == null) && errorProperty == null)
		{
			...
			OutputStreamFunneler funneler = new OutputStreamFunneler(outputStream, funnelTimeout);
			try
			{
	 ...
	 * values.
	 *
	 * @throws IOException if the output properties cannot be read from their output
	 *                     streams.
	 */
	public void complete() throws IOException
	{
		...
		synchronized(this)
		{
			while(threadGroup.activeCount() > 0)
			{
				try
				{
					managingTask.log("waiting for " + threadGroup.activeCount() + " Threads:", Project.MSG_DEBUG);
