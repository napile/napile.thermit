 ...
 *
 */
package org.napile.thermit.taskdefs.optional.depend;
...
import org.napile.thermit.taskdefs.rmic.DefaultRmicAdapter;
import org.napile.thermit.taskdefs.rmic.WLRmic;
import org.napile.thermit.types.Path;
import org.napile.thermit.types.Reference;
import org.napile.thermit.util.FileUtils;
import org.napile.thermit.util.depend.DependencyAnalyzer;
...
public class Depend extends MatchingTask
{
	 ...
	 * @throws IOException if the dependency file cannot be written out.
	 */
	 ...
	 *
	 * @throws IOException if either the dependencies cache or the class
	...
	private int deleteAffectedFiles(String className)
	{
		...
		for(Enumeration e = affectedClasses.keys(); e.hasMoreElements(); )
		{
			...
			log("Deleting file " + affectedClassInfo.absoluteFile.getPath() + " since " + className + " out of date", Project.MSG_VERBOSE);
			...
			else
			{
				...
				ClassFileInfo topLevelClassInfo = (ClassFileInfo) classFileInfoMap.get(topLevelClassName);
				if(topLevelClassInfo != null && topLevelClassInfo.absoluteFile.exists())
				{
					log("Deleting file " + topLevelClassInfo.absoluteFile.getPath() + " since one of its inner classes was removed", Project.MSG_VERBOSE);
	...
	{
		return isStub(affectedClass, className, DefaultRmicAdapter.RMI_STUB_SUFFIX) || isStub(affectedClass, className, DefaultRmicAdapter.RMI_SKEL_SUFFIX) || isStub(affectedClass, className, WLRmic.RMI_STUB_SUFFIX) || isStub(affectedClass, className, WLRmic.RMI_SKEL_SUFFIX);
	}
