...
package org.napile.thermit.taskdefs.compilers;

//Java5 style
//import static org.napile.thermit.util.StringUtils.LINE_SEP;
...
import org.napile.thermit.taskdefs.Javac;
...
public abstract class DefaultCompilerAdapter implements CompilerAdapter, CompilerAdapterExtension
{
	...
	protected Commandline setupModernJavacCommandlineSwitches(Commandline cmd)
	{
		...
		if(!assumeJava13())
		{ // -source added with JDK 1.4
			final String t = attributes.getTarget();
			if(attributes.getSource() != null)
			{
				cmd.createArgument().setValue("-source");
				cmd.createArgument().setValue(adjustSourceValue(attributes.getSource()));
	...
	protected int executeExternalCompile(String[] args, int firstFileName, boolean quoteFiles)
	{
		...
		{			/*
			...
			if(Commandline.toString(args).length() > COMMAND_LINE_LIMIT && firstFileName >= 0)
			{
				...
				try
				{
					tmpFile = FILE_UTILS.createTempFile("files", "", getJavac().getTempdir(), true, true);
			...
			{
				throw new BuildException("Error running " + args[0] + " compiler", e, location);
			}
		}
		finally
		{
	 ...
	 *             Use org.napile.thermit.types.Path#addExtdirs instead.
	 */
	...
	{
		return javacXY.equals(attributes.getCompilerVersion()) || ("classic".equals(attributes.getCompilerVersion()) && JavaEnvUtils.isJavaVersion(javaEnvVersionXY)) || ("modern".equals(attributes.getCompilerVersion()) && JavaEnvUtils.isJavaVersion(javaEnvVersionXY)) || ("extJavac".equals(attributes.getCompilerVersion()) && JavaEnvUtils.isJavaVersion(javaEnvVersionXY));
	}
	...
	private void setImplicitSourceSwitch(Commandline cmd, String target, String source)
	{
		...
		attributes.log("", Project.MSG_WARN);
		attributes.log("The -source switch defaults to " + getDefaultSource() + ".", Project.MSG_WARN);
		attributes.log("If you specify -target " + target + " you now must also specify -source " + source + ".", Project.MSG_WARN);
		attributes.log("Ant will implicitly add -source " + source + " for you.  Please change your build file.", Project.MSG_WARN);
		cmd.createArgument().setValue("-source");
		cmd.createArgument().setValue(source);
	}
	...
	private boolean mustSetSourceForTarget(String t)
	{
		...
		return t.equals("1") || t.equals("2") || t.equals("3") || t.equals("4") || ((t.equals("5") || t.equals("6")) && !assumeJava15() && !assumeJava16()) || (t.equals("7") && !assumeJava17());
	}
