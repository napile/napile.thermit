...
import org.napile.thermit.taskdefs.Rmic;
...
public abstract class DefaultRmicAdapter implements RmicAdapter
{
	...
	protected String addStubVersionOptions()
	{
		...
		if(stubOption == null && !attributes.getIiop() && !attributes.getIdl())
		{
			stubOption = STUB_COMPAT;
		}
		return stubOption;
	...
	private class RmicFileNameMapper implements FileNameMapper
	{
		...
		{
			if(name == null || !name.endsWith(".class") || name.endsWith(getStubClassSuffix() + ".class") || name.endsWith(getSkelClassSuffix() + ".class") || name.endsWith(getTieClassSuffix() + ".class"))
			{
			...
			else if(!attributes.getIdl())
			{
				...
				{
					attributes.log("Unable to verify class " + classname + ". It could not be found.", Project.MSG_WARN);
				}
				catch(NoClassDefFoundError e)
				{
					attributes.log("Unable to verify class " + classname + ". It is not defined.", Project.MSG_WARN);
				}
				catch(Throwable t)
				{
					attributes.log("Unable to verify class " + classname + ". Loading caused Exception: " + t.getMessage(), Project.MSG_WARN);
				}
			}
			return target;
		}
