 ...
 *
 */
package org.napile.thermit.taskdefs.optional.script;

import org.napile.thermit.AntTypeDefinition;
import org.napile.thermit.ComponentHelper;
...
import org.napile.thermit.taskdefs.DefBase;
...
import org.napile.thermit.util.ScriptRunnerBase;
import org.napile.thermit.util.ScriptRunnerHelper;
...
public class ScriptDef extends DefBase
{
	...
	public void execute()
	{
		...
		for(Iterator i = attributes.iterator(); i.hasNext(); )
		{
			...
			{
				throw new BuildException("scriptdef <" + name + "> declares " + "the " + attribute.name + " attribute more than once");
			}
		...
		for(Iterator i = nestedElements.iterator(); i.hasNext(); )
		{
			...
			{
				throw new BuildException("scriptdef <" + name + "> declares " + "the " + nestedElement.name + " nested element more " + "than once");
			}
	...
	public Object createNestedElement(String elementName)
	{
		...
		if(definition == null)
		{
			throw new BuildException("<" + name + "> does not support " + "the <" + elementName + "> nested element");
		}
		...
		if(instance == null)
		{
			throw new BuildException("<" + name + "> is unable to create " + "the <" + elementName + "> nested element");
		}
		return instance;
	}
