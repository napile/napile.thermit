 ...
 *
 */
package org.napile.thermit.taskdefs.optional;
...
import org.napile.thermit.types.RegularExpression;
import org.napile.thermit.types.Resource;
import org.napile.thermit.types.ResourceCollection;
import org.napile.thermit.types.Substitution;
...
import org.napile.thermit.util.regexp.Regexp;
import org.napile.thermit.util.regexp.RegexpUtil;
 ...
 * you choose to use. The system property <code>thermit.regexp.regexpimpl</code>
 * will be the classname of the implementation that will be used (the default
 * is <code>org.napile.thermit.util.regexp.JakartaOroRegexp</code> and
 * requires the Jakarta Oro Package).
 * <p/>
 * <pre>
 * Available implementations:
 *
 *   org.napile.thermit.util.regexp.Jdk14RegexpRegexp (default)
 *        Uses Java's built-in regular expression package
 *
 *   org.napile.thermit.util.regexp.JakartaOroRegexp
 *        Requires  the jakarta-oro package
 *
 *   org.napile.thermit.util.regexp.JakartaRegexpRegexp
...
public class ReplaceRegExp extends Task
{
		...
		try
		{
			...
			try
			{
				...
				try
				{
					...
					log("Replacing pattern '" + regex.getPattern(getProject()) + "' with '" + subs.getExpression(getProject()) + "' in '" + f.getPath() + "'" + (byline ? " by line" : "") + (flags.length() > 0 ? " with flags: '" + flags + "'" : "") + ".", Project.MSG_VERBOSE);
	...
	public void execute() throws BuildException
	{
		...
		if(file != null && file.exists())
		{
			...
			{
				log("An error occurred processing file: '" + file.getAbsolutePath() + "': " + e.toString(), Project.MSG_ERR);
			}
		}
		else if(file != null)
		{
			log("The following file is missing: '" + file.getAbsolutePath() + "'", Project.MSG_ERR);
		}
		...
		if(resources != null)
		{
			for(Resource r : resources)
			{
				...
				if(f.exists())
				{
					...
					{
						log("An error occurred processing file: '" + f.getAbsolutePath() + "': " + e.toString(), Project.MSG_ERR);
					}
				}
				else
				{
					log("The following file is missing: '" + f.getAbsolutePath() + "'", Project.MSG_ERR);
				}
			}
		}
	}
