...
import org.napile.thermit.types.ArchiveFileSet;
...
import org.napile.thermit.types.resources.ArchiveResource;
...
import org.napile.thermit.types.resources.TarResource;
import org.napile.thermit.util.FileUtils;
import org.napile.thermit.util.MergingMapper;
import org.napile.thermit.util.ResourceUtils;
import org.napile.thermit.util.SourceFileScanner;
import org.napile.thermit.util.bzip2.CBZip2OutputStream;
import org.napile.thermit.util.tar.TarConstants;
import org.napile.thermit.util.tar.TarEntry;
import org.napile.thermit.util.tar.TarOutputStream;
...
public class Tar extends MatchingTask
{
	...
	public void execute() throws BuildException
	{
		...
		try
		{
			...
			{
				throw new BuildException("Failed to create missing parent" + " directory for " + tarFile);
			}
	...
	{
		return SelectorUtils.isOutOfDate(new FileResource(tarFile), r, FileUtils.getFileUtils().getFileTimestampGranularity());
	}
	...
	protected boolean check(ResourceCollection rc)
	{
		...
		else if(rc.isFilesystemOnly())
		{
			...
			while(iter.hasNext())
			{
				...
				upToDate &= check(base == Copy.NULL_FILE_PLACEHOLDER ? null : base, files);
			}
		}
		else
		{ // non-file resources
	...
	protected TarFileSet asTarFileSet(ArchiveFileSet archiveFileSet)
	{
		...
		else
		{
			...
			if(archiveFileSet != null)
			{
				...
				if(archiveFileSet instanceof org.napile.thermit.types.TarFileSet)
				{
					org.napile.thermit.types.TarFileSet t = (org.napile.thermit.types.TarFileSet) archiveFileSet;
	 ...
	 */
	public static class TarFileSet extends org.napile.thermit.types.TarFileSet
	{
	...
	public static class TarLongFileMode extends EnumeratedAttribute
	{
		 ...
		 */
		public static final String WARN = "warn", FAIL = "fail", TRUNCATE = "truncate", GNU = "gnu", POSIX = "posix", OMIT = "omit";
