 ...
 *             delegate to {@link org.napile.thermit.taskdefs.Execute Execute}
 *             instead.
 */
public class Exec extends Task
{
	...
	protected int run(String command) throws BuildException
	{
		...
		if(myos.toLowerCase(Locale.ENGLISH).indexOf("windows") >= 0)
		{
			if(!dir.equals(getProject().resolveFile(".")))
			{
				...
				else
				{
					...
					if(ant == null)
					{
						throw new BuildException("Property '" + MagicNames.ANT_HOME + "' not " + "found", getLocation());
					}
		...
		try
		{
			...
			StreamPumper errorPumper = new StreamPumper(proc.getErrorStream(), Project.MSG_WARN);
