 ...
 *
 */
package org.napile.thermit.taskdefs.optional.extension;
...
import org.napile.thermit.taskdefs.optional.extension.resolvers.AntResolver;
import org.napile.thermit.taskdefs.optional.extension.resolvers.LocationResolver;
import org.napile.thermit.taskdefs.optional.extension.resolvers.URLResolver;
...
public class JarLibResolveTask extends Task
{
	...
	public void execute() throws BuildException
	{
		...
		{
			final ExtensionResolver resolver = (ExtensionResolver) resolvers.get(i);

			getProject().log("Searching for extension using Resolver:" + resolver, Project.MSG_VERBOSE);
			...
			{
				final String message = "Failed to resolve extension to file " + "using resolver " + resolver + " due to: " + be;
				getProject().log(message, Project.MSG_WARN);
			}
		}
		missingExtension();
	...
	private void checkExtension(final File file)
	{
		...
		if(!checkExtension)
		{
			getProject().log("Setting property to " + file + " without verifying library satisfies extension", Project.MSG_VERBOSE);
			setLibraryProperty(file);
		}
		else
		{
			getProject().log("Checking file " + file + " to see if it satisfies extension", Project.MSG_VERBOSE);
